...
test('calls.resolveCallerName resolves primary, secondary, fallback', () => {
    ...
    assert.equal(resolveCallerName({
...
test('calls.getTimeGreeting returns expected greeting by hour', () => {
    ...
    assert.equal(getTimeGreeting({ timeZone: 'UTC', now: morning }), 'Good morning');
    assert.equal(getTimeGreeting({ timeZone: 'UTC', now: afternoon }), 'Good afternoon');
    assert.equal(getTimeGreeting({ timeZone: 'UTC', now: evening }), 'Good evening');
});
