...
/**
 * Create a Text Search tool (Places API (New)) with in-memory caching.
 *
 * Uses Places API (New) HTTP endpoint `places:searchText` and a FieldMask to keep responses lean.
 *
 * @param {object} opts - Tool configuration.
 * @param {string} opts.apiKey Google Maps Platform API key.
 * @param {number=} opts.ttlMs Cache TTL in ms. Default: 150000 (2.5 minutes).
 * @param {string[]=} opts.fieldMask Places field mask. Default is a minimal set.
 * @returns {(args: GooglePlacesTextSearchArgs) => Promise<{places: TextSearchPlace[]}|null>} Text search tool.
 */
export function createGooglePlacesTextSearchTool(opts) {
    const apiKey = String(opts?.apiKey || '');
    const ttlMs = Number.isFinite(opts?.ttlMs) ? opts.ttlMs : 150000;

    const fieldMask =
        Array.isArray(opts?.fieldMask) && opts.fieldMask.length
            ? opts.fieldMask
            : [
                  'places.id',
                  'places.displayName',
        ...
        const restrict = args.locationRestriction
            ? {
                  center: {
                      lat:
                          Math.round(
                              args.locationRestriction.center.lat * 1e6
                          ) / 1e6,
                      lng:
                          Math.round(
                              args.locationRestriction.center.lng * 1e6
                          ) / 1e6,
                  },
    ...
    /**
     * Text Search (New) via Places API (New) HTTP endpoint.
     *
     * @param {GooglePlacesTextSearchArgs} args - Tool input args.
     * @returns {Promise<{places: TextSearchPlace[]}|null>} Returns null on any failure.
     */
    return async function googlePlacesTextSearch(args) {
        try {
